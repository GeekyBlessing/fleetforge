...
Heartbeats update `worker:{id}:state` and `worker:{id}:alive` synchronously on every 5s tick (2,000 ops/sec sustained at 10k workers — trivial for Redis). A background writer flushes `last_heartbeat` and `status` to Postgres on a coalesced schedule: at most once per worker per 15 seconds, or immediately on any *state transition* (READY→BUSY, BUSY→READY, →DRAINING, →DEAD). This means Postgres sees every status change in real time but doesn't take a write for every single unchanged heartbeat — cutting sustained heartbeat-driven Postgres writes from ~2,000/sec to a small fraction of that, only on actual transitions plus a low-rate keepalive.
