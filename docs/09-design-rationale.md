...
The scheduling requirements name six approaches: Round Robin, Least Loaded, Priority Scheduling, Capability Matching, Worker Labels/Affinity, Queue Priorities. These aren't six competing algorithms to choose one of: they're a **pipeline**, and the interesting design decision is the order:
...
**What could be added later, explicitly not in the current scope:** true affinity/anti-affinity rules beyond simple label-equality matching, for example preferring a worker that already has a repo's Docker layer cache warm. That requires tracking cache-locality state per worker, which is a real optimization for build systems specifically (akin to Bazel remote-cache-aware scheduling), but it's a substantial feature on its own.
...
- **Idle workers** (READY with full available_capacity): direct "do we have slack" signal; prevents scaling up when there's actually capacity sitting unused (e.g. queue depth is high only because of a capability mismatch, where more workers of the *wrong* kind won't help, which is exactly why idle-worker count is tracked per capability/label combination, not just as a fleet-wide total).
...
## 9.7 Open questions and assumptions

1. **Log storage backend.** Doc 3 assumes `log_ref` points to external object storage (S3-compatible). Absent an existing bucket/MinIO setup, the initial implementation defaults to local disk with a documented swap point.
2. **Cloud provider for autoscaling.** Doc 9.2/doc 7 stub an AWS ASG provider as the concrete example. GCP, Nomad, or a purely on-prem/manual fleet (where the autoscaler is a no-op that only emits recommendations) are equally valid targets behind the same `FleetProvider` interface.
3. **Realistic load-test target.** 500-1,000 simulated workers is the validated number; the 10,000 figure is a documented extrapolation rather than something actually spun up during implementation.
4. **Integration with existing infrastructure.** An existing Postgres cluster, secrets manager for the mTLS CA, or CI system webhook calling `POST /jobs` would shape the initial scaffold; absent that, the default assumption is a from-scratch docker-compose environment.

---

This closes the architecture and design phase (docs 1-9, plus the 00-README index). Implementation followed the milestones laid out in doc 8.
